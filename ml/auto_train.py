...
class TrainingManager:
    ...
    def _blank(self,status):
        return {
            "status":status,"version":self.version,"started_at":None,
            "finished_at":None,"error":None,"dataset_rows":0,
    ...
    def _persist(self,symbol):
        try: save_training_state(symbol,self.state[symbol])
    ...
    async def _fit_validate(self,symbol,dataset,rr,horizon,source,history=None):
        ...
        if any(len(np.unique(x["label"]))<2 for x in (train,cal,test)):
        ...
        self.registry.save(
            symbol,self.version,bundle,
            {**metrics,"trained_at":trained_at,"mode":"SHADOW_RESEARCH"},
        )
        self._update(
            symbol,status="READY",finished_at=trained_at,
            dataset_rows=int(len(dataset)),metrics=metrics,
            quality=gate,error=None,
        )
        return gate

    async def _train_btc(self):
        self._update(
            "BTCUSD",status="DOWNLOADING_HISTORY",
        ...
        try:
            ...
            if len(m3)<5000 or len(m5)<2500 or len(m15)<900:
                raise RuntimeError(
            ...
            await self._fit_validate(
                "BTCUSD",dataset,rr,horizon,
                source="Coinbase Exchange BTC-USD",
                history={
                    "days":days,
                    "M3":int(len(m3)),"M5":int(len(m5)),"M15":int(len(m15)),
                },
            )
    ...
    async def _train_xau(self):
        history=market_history_status("XAUUSD")
        self._update(
            "XAUUSD",status="BUILDING_DATASET",
            started_at=datetime.now(timezone.utc).isoformat(),
            finished_at=None,error=None,dataset_rows=0,metrics=None,
            quality=None,history=history,
        )
        try:
            rr=float(os.getenv("DEFAULT_RR","3.0"))
            ...
            if len(m3)<10000 or len(m5)<5000 or len(m15)<1500:
                raise RuntimeError(
                    f"insufficient MT5 history: M3={len(m3)} M5={len(m5)} M15={len(m15)}"
                )

            dataset=await asyncio.to_thread(build,m3,m5,m15,rr,horizon)
            self._update("XAUUSD",dataset_rows=int(len(dataset)))
            await self._fit_validate(
                "XAUUSD",dataset,rr,horizon,
